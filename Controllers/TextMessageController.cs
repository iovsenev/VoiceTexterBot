        ...
        public async Task Handle(Message message, CancellationToken ct)
        {
            switch (message.Text)
            {
                case "/start":
                    ...
                    buttons.Add(new[]
                    {
                        InlineKeyboardButton.WithCallbackData($" :ru: Русский" , $"ru"),
                        InlineKeyboardButton.WithCallbackData($" :gb: English" , $"en")
                    });

                    // передаем кнопки вместе с сообщением (параметр ReplyMarkup)
                    await _telegramClient.SendTextMessageAsync(message.Chat.Id, $":studio_microphone:<b>  Наш бот превращает аудио в текст.</b> {Environment.NewLine}" +
